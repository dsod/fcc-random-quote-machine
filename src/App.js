...
class App extends React.Component {
  ...
  renderNewQuote() {
    const randomQuoteIndex = Math.floor(
      Math.random() * this.state.quotes.length - 1
    );
    return this.state.quotes[randomQuoteIndex];
  }
  ...
  render() {
    const renderedQuote = this.renderNewQuote();
    return (
        ...
        <div className="row vh-100 align-items-center justify-content-center">
          ...
          >
            <div className="card-body">
              <h2 id="text">
                "<small className="text-muted">{renderedQuote.quoteText}</small>
                "
              </h2>
              <p className="lead text-right" id="author">
                - {renderedQuote.quoteAuthor}
              </p>
